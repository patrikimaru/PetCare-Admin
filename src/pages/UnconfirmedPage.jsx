...
const UnconfirmedPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/appointments/unconfirmed', {
          headers: {
  ...
  const handleConfirm = async (appointmentId) => {
    const confirmed = window.confirm('Are you sure you want to confirm this appointment?');
    if (!confirmed) {
      return;
    }

    try {
      await axios.put(`http://localhost:3001/api/appointments/confirm/${appointmentId}`,{}, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        ...
        <tbody>
          {filteredAppointments.map((appointment) => (
            <tr key={appointment._id}>
              ...
              <td>
                <button onClick={() => handleConfirm(appointment._id)} className='add-btn'>Confirm</button>
              </td>
            </tr>
          ))}
        </tbody>
